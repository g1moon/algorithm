...
for hw_url in hw_urls:
    ...
    for i in range(0, rows_len):
        if '미제출' in bs.select('#region-main > div > table > tbody')[0].select('tr')[i].text:
            ...
            if '[HW2]프로그램 3.1의 확장' == temp_row[1]:
                Deadline_list.append('4월9일(목) 오후 11:50까지')
            elif '[과제3] 파일 입출력 프로그램 확장' == temp_row[1]:
                Deadline_list.append('4/23(목) 오후11시50분까지')
            else:
